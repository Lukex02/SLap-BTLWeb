  ...
  products.forEach((product, index) => {
    const row = `
      <tr>
        <td>${index + 1}</td>
        <td>
          <img src="${product.image}" alt="${product.name}" class="product-thumbnail" width="60">
        </td>
        <td>${product.name}</td>
        <td>${product.brand}</td>
        <td>${formatCurrency(product.price)}đ</td>
        <td>
          <button class="btn btn-info btn-sm view-btn" onclick="viewProduct(${product.id})">Xem</button>
...
function viewProduct(id) {
  ...
  document.getElementById("view-product-price").textContent = formatCurrency(product.price) + "đ";
      ...
      fetch(`/server/deleteProduct.php?id=${id}`)
        ...
        .then((data) => {
          if (data.success) {
            ...
            showAlert(data.message || "Có lỗi xảy ra khi xóa sản phẩm!", "error");
          }
        })
        .catch((error) => {
          console.error("Lỗi:", error);
